...
Name: Nader Tarek
Email: nader.tarek.elsherbiny@gmail.com
...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned i = 0; i < image.width(); i++)
  {
    ...
    //  int diffX = abs(centerX - i);
    //  int diffY = abs(centerY - j);
    //  int powX = pow(diffX, 2);
    //  int powY = pow(diffY, 2);
    //  int euclideanDist = sqrt(powX + powY);

    //  if (euclideanDist > 160)
    //  {
    //    pixel.l = pixel.l - (80/100) * pixel.l;
    //  }
    //  else
    //  {
    //    double luminanceDec = euclideanDist * 0.005;
    //    pixel.l =  pixel.l - (euclideanDist/100) * pixel.l;
    //  }
     int hor = centerX - i;
